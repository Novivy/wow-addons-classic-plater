		...
		function Plater.CastBarOnEvent_Hook (self, event, unit, ...) --private
			...
			if (self.isNamePlate) then
				...
				elseif (event == "UNIT_SPELLCAST_INTERRUPTED") then
					...
					-- Always mark as interrupted so cast scripts have correct context regardless of show_interrupt_author
					self.IsInterrupted = true

					-- Fallback: if the DF handler didn't catch it (self.casting was already nil, e.g. bar finished
					-- early due to LibCC default 1500ms cast time), force the interrupt visual state here.
					-- OnHideWidget is intentionally NOT called here because UNIT_SPELLCAST_STOP fires right after
					-- and will call it once.
	...
	local parserFunctions = {
		...
		SPELL_INTERRUPT = function (time, token, hidding, sourceGUID, sourceName, sourceFlag, sourceFlag2, targetGUID, targetName, targetFlag, targetFlag2, spellID, spellName, spellType, amount, overKill, school, resisted, blocked, absorbed, isCritical)
			...
			for _, plateFrame in ipairs (Plater.GetAllShownPlates()) do
				local unitFrame = plateFrame.unitFrame
				local castBar = unitFrame.castBar
				-- Include bars that were recently active (may already be fading) in addition to currently shown ones
				local wasRecentlyCasting = castBar.casting or castBar.channeling or (castBar.finished and not castBar.interrupted)
				if (unitFrame.PlaterOnScreen and (castBar:IsShown() or wasRecentlyCasting)) then
					if (plateFrame [MEMBER_GUID] == targetGUID) then

						-- Always force the interrupt visual state, covering both regular casts and channeled spells.
						-- Channeled spells only receive UNIT_SPELLCAST_CHANNEL_STOP from LibCC (not INTERRUPTED),
						-- so they must be handled here directly.
						if (not castBar.interrupted) then
							castBar.casting = nil
							castBar.channeling = nil
							castBar.interrupted = true
							castBar.finished = true
							castBar.castID = nil
							castBar:Animation_StopAllAnimations()
							castBar:CancelScheduleToHide()
							castBar:SetAlpha(1)
							castBar:Show()
							if (castBar.Settings.FillOnInterrupt) then
								castBar:SetValue(castBar.maxValue or select(2, castBar:GetMinMaxValues()) or 1)
							end
							if (castBar.Settings.HideSparkOnInterrupt) then
								castBar.Spark:Hide()
							end
							castBar:UpdateCastColor()
							castBar.percentText:Hide()
							castBar.Text:SetText(INTERRUPTED)
							castBar:ScheduleToHide(1)
						end
