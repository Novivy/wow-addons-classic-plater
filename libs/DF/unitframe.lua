...
detailsFramework.CastFrameFunctions = {
	...
	UNIT_SPELLCAST_INTERRUPTED = function(self, unit, ...)
		...
		-- Allow interrupt even when cast already "finished" locally (e.g. LibCC used wrong default cast time)
		-- but skip if already interrupted or if there was never any cast tracked at all.
		local wasActive = self.casting or self.channeling
		local finishedEarly = self.finished and not self.interrupted
		if ((wasActive or finishedEarly) and castID == self.castID) then
			-- Stop any in-progress fade animation so the bar is visible for the interrupt flash
